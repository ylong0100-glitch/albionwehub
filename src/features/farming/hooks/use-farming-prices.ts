...
import { getAllFarmingItemIds } from '../utils/farming-data'
import type { PriceEntry } from '@/lib/api/albion-data'

interface UseFarmingPricesReturn {
...
/**
 * Batch fetch prices for all farming items (crops + seeds) in a single API call.
 * Returns a map of itemId -> sell_price_min (lowest across all cities).
 */
export function useFarmingPrices(): UseFarmingPricesReturn {
  ...
  const fetchPrices = useCallback(async () => {
    ...
    try {
      const itemIds = getAllFarmingItemIds()
      ...
      const message =
        err instanceof Error ? err.message : 'Failed to fetch farming prices'
      if (!controller.signal.aborted) {
        setError(message)
      }
    } finally {
